...
**Bảng trượt xác nhận "Thêm giỏ hàng" (`showSlideToast()`, vòng sửa 25)** — thay thế hẳn việc mở modal to giữa màn hình mỗi lần bấm "Thêm giỏ hàng": 1 bảng nhỏ (`.cart-slide-toast`, nền trắng, viền cam bên trái) trượt vào từ MÉP TRÁI màn hình (neo `top:110px; left:20px`, không phải bottom-center như `.toast` thường), tự ẩn sau **2 giây** (`setTimeout` 2000ms, khác với `showToast()` thường là 2.6s). Dùng chung 1 `<div id="swiftstreet-slide-toast">` tái sử dụng như `showToast()`. Trên mobile (`max-width:480px`) bảng giãn full chiều ngang (trừ lề 12px) thay vì giữ `max-width:300px` cố định.
...
- **Có `?slug=<slug>`** (bấm "Mua ngay" ở thẻ sản phẩm/trang chi tiết sản phẩm, KHÔNG qua giỏ hàng): `getCheckoutItems()` trả về đúng 1 sản phẩm (tra trực tiếp trong `PRODUCTS`, kèm `priceOld`/`shortDesc` để hiển thị giá gạch ngang — không đụng tới giỏ hàng thật). KHÔNG hiện ô "Chọn tất cả", KHÔNG có checkbox/nút xoá từng dòng (chỉ 1 sản phẩm, không có ý nghĩa chọn/xoá).
- **Không có `?slug=`** (bấm icon giỏ hàng ở header, hoặc "Mua ngay" trong thẻ sản phẩm khi đã ở trang thanh toán): render TOÀN BỘ `getCart()` (kể cả sản phẩm đang BỎ CHỌN — vẫn hiện, chỉ mờ đi `opacity:0.5` qua class `.is-unselected`, để khách còn tick lại được), kèm ô **"Chọn tất cả"** (`.cart-select-all`, ẩn/hiện qua `#checkout-select-all`) + checkbox từng dòng (`data-checkout-toggle-select`) + icon xoá (`data-checkout-remove`, xoá luôn khỏi giỏ hàng thật). **Chỉ những item có `selected:true` mới được cộng vào "Cần thanh toán"** — đây là lỗi thực tế đã gặp và sửa trong vòng 25: ban đầu dùng nhầm `getCheckoutItems()` (vốn đã lọc sẵn `selected`) để RENDER danh sách, khiến bỏ chọn 1 sản phẩm làm nó biến mất khỏi danh sách luôn thay vì chỉ mờ đi, và khiến "Chọn tất cả" luôn hiện đã chọn dù vừa bỏ chọn 1 món (vì mảng đã lọc sẵn nên `.every(selected)` luôn đúng). Đã sửa: `render()` dùng `getCart()` đầy đủ để vẽ danh sách, rồi mới `.filter(item => item.selected)` riêng để tính tổng tiền.
- Danh sách sản phẩm (`.checkout-product-list`) hiện **tối đa 5 dòng cùng lúc** (`max-height:636px`, đo đúng bằng chiều cao 1 dòng thực tế × 5 + gap), quá 5 thì tự cuộn RIÊNG danh sách (`overflow-y:auto`) — không cuộn cả trang.
...
- Bố cục 2 cột (`.checkout-grid`, `grid-template-columns: minmax(0, 1.1fr) minmax(0, 1fr)` — **bắt buộc dùng `minmax(0, ...)` chứ không phải `1.1fr 1fr`/`1fr` trần**, xem lỗi tràn ngang trên mobile bên dưới):
  - **Cột trái**: "Chọn tất cả" (nếu multi) → danh sách sản phẩm, mỗi dòng đóng khung riêng (`.checkout-item-row`, border+radius, KHÔNG còn kiểu "hàng có border-bottom" như modal cũ) gồm checkbox + tên (link sang trang chi tiết NẾU sản phẩm đó có trang thật — xem `CHECKOUT_LINKABLE_SLUGS`, hiện chỉ `swiftcopy-drive`) + mô tả ngắn + giá hiện tại VÀ giá gốc gạch ngang xếp DỌC bên dưới (`.item-price-stack`, khác modal cũ là xếp ngang) + icon xoá → tiếp theo là "Điền thông tin của bạn" với 3 ô nhập (`.checkout-input-form`, các input/label giới hạn `max-width:260px` — KHÔNG kéo dài hết cột như `.simple-form` cũ) → đoạn ghi chú email/SĐT.
  - **Cột phải**: 2 khối TÁCH RIÊNG (yêu cầu tường minh vòng 25, trước đó gộp chung 1 khối):
    1. **`.checkout-summary-box`** "Thông tin đơn hàng": Tổng tiền (tổng giá GỐC), Tổng khuyến mãi (tổng phần chênh lệch, màu đỏ `var(--color-error)`), Voucher (dòng có chevron `▾` — bấm `#voucher-toggle` toggle class `.open` (xoay chevron 180°) + hiện/ẩn `#voucher-input-wrap` — CHƯA xử lý mã thật, chỉ là UI), và dòng "Cần thanh toán" đậm + màu cam (`.total-amount`, `var(--color-orange-dark)`, 18px).
    2. **`.checkout-bank-box`** "Thông tin chuyển khoản": `.payment-mock` (đã đổi nền TRẮNG/xám nhạt thay vì đen — xem mục riêng bên dưới, KHÔNG còn dòng "Số tiền" vì đã hiển thị ở "Cần thanh toán" phía trên rồi) + nút "Tôi đã chuyển khoản" (`.checkout-paid-btn`, `max-width:220px` căn giữa — KHÔNG còn `btn-block` full-width dẹt mảnh như trước) + đoạn ghi chú cuối `.checkout-final-note` (nguyên văn đã chốt, có 3 cụm gạch chân/link thật: "Điều khoản sử dụng" → `footer-pages/dieu-khoan-su-dung.html`, "Khuyến mãi" → `khuyen-mai.html`, "Chính sách đổi trả dịch vụ" → `footer-pages/chinh-sach-doi-tra.html`, dùng `<a class="note-link">` với `text-decoration:underline; color:inherit` — KHÔNG đổi màu xanh mặc định).
- **KHÔNG còn badge "THANH TOÁN" phía trên tiêu đề** (đã bỏ hẳn) — chỉ còn `<h1 class="checkout-heading">Hoàn tất đơn hàng</h1>`, và bỏ khoảng trắng thừa dưới breadcrumb (không còn `.section-heading` bọc ngoài + margin-bottom 32px như trước).
...
### Trang placeholder "Khuyến mãi"/"Kiếm Tiền" — bỏ khoảng trắng thừa (vòng sửa 25)
...
3. Nối luồng thanh toán thật: nút "Tôi đã chuyển khoản" ở `thanh-toan.html`, ô nhập mã voucher, và form "Đăng ký" ở trang CTV hiện chưa gửi dữ liệu đi đâu — cần backend (Firebase) để lưu đơn hàng/áp mã/đăng ký CTV thật.
